...
function TrustItem({ label, icon: Icon }: { label: React.ReactNode; icon: typeof Star }) {
  return (
    <div className="flex min-w-0 items-center gap-2 rounded-xl bg-white/65 px-2 py-4 text-left shadow-[0_8px_22px_rgba(122,63,33,.08)] backdrop-blur-[2px] sm:gap-[.8vw] sm:rounded-2xl sm:px-[.9vw] sm:py-[.75vw]">
      ...
      <span className="text-[13px] font-bold leading-[1.15] text-[#062e5a] sm:text-[clamp(15px,1.03vw,20px)]">{label}</span>
    </div>
  )
}
    ...
    <section className="relative isolate overflow-hidden bg-[#002e5c] font-[var(--font-merriweather)]">
      ...
      <div className="relative mx-auto min-h-[700px] max-w-[1920px] sm:min-h-0 sm:aspect-[2.285/1] sm:px-[7vw] sm:py-0">
        ...
        <div className="absolute left-5 top-[6%] z-10 w-[90%] sm:left-[10.3%] sm:top-[18.5%] sm:w-[36%] sm:max-w-none">
          <p className="!m-0 text-[21px] font-black leading-[1.1] tracking-[-.055em] text-[#002c5b] sm:text-2xl">
            #1 Hair Care in Chennai , Mylapore
          </p>
        ...
        {/* Contact block */}
        <div className="absolute left-1/2 top-[49%] z-20 w-[calc(100%-2.5rem)] -translate-x-1/2 text-center text-white max-[359px]:top-[54%] sm:bottom-[12.5%] sm:left-auto sm:right-[3.1%] sm:top-auto sm:w-fit sm:translate-x-0 sm:text-left">
          <a href="#assessment-form" className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#002c5b] px-4 py-2.5 text-[13px] font-black text-white transition hover:scale-[1.02] sm:gap-[1vw] sm:bg-[linear-gradient(90deg,#ffc69f,#ffc096)] sm:px-[1.3vw] sm:py-[.85vw] sm:text-[clamp(11px,1.15vw,22px)] sm:text-[#003061]">
