  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes achievement-dot-pulse {
          0%, 100% { opacity: 0.35; transform: scale(0.8); }
          50% { opacity: 1; transform: scale(1); }
        }
        @keyframes achievement-line-sweep {
          0% { left: -35%; }
          100% { left: 100%; }
        }
      `}</style>
      <div className="mx-auto max-w-[1200px] text-center">
        <h2 id="achievement-heading" className="m-0 [font-family:Arial,Helvetica,sans-serif] text-[21px] leading-[26px] font-bold tracking-[0.25px]">Clinic Achievements</h2>

        <div className="mx-auto mt-3 flex items-center justify-center gap-2" aria-hidden="true">
          <span
            className="h-[6px] w-[6px] rounded-full bg-[#FBA062]"
            style={{ animation: "achievement-dot-pulse 1.6s ease-in-out infinite" }}
          />
          <span className="relative h-[3px] w-[56px] overflow-hidden rounded-full bg-[#FBA062]/20">
            <span
              className="absolute inset-y-0 w-1/3 rounded-full bg-[#FBA062]"
              style={{ animation: "achievement-line-sweep 2.2s ease-in-out infinite" }}
            />
          </span>
          <span
            className="h-[6px] w-[6px] rounded-full bg-[#FBA062]"
            style={{ animation: "achievement-dot-pulse 1.6s ease-in-out infinite", animationDelay: "0.7s" }}
          />
        </div>
